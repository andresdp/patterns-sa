...
class PRIMDiscovery(ScenarioDiscovery):
    def discover(self, experiments_df: pd.DataFrame, outcomes_df: pd.DataFrame, **kwargs) -> Any:
        ...
        Runs PRIM analysis.
        kwargs:
            property: tuple/dict describing the ROI (e.g. {'response_time': (min, max)})
            threshold: float
            method: 'rhodium' or 'ema'
            key_parameters: list of strings
            n_boxes: bool (return all boxes or just first)
            verbose: bool
        """
        ...
        # Prepare data
        # Assuming outcomes_df has the relevant columns.
        # property is e.g., {'response_time': (min, max), 'utilization': (min, max)}
        
        if property is None:
             raise ValueError("Property (ROI) must be defined for PRIM")

        # Construct y (ROI mask)
...
class CARTDiscovery(ScenarioDiscovery):
    def discover(self, experiments_df: pd.DataFrame, outcomes_df: pd.DataFrame, **kwargs) -> Any:
        ...
        Runs CART analysis.
        kwargs:
            key_parameters: list of str
            prune_tree: bool
            mass_min: float
            discrete_outcomes: Series/DataFrame of class labels (required)
        """
        key_parameters = kwargs.get('key_parameters')
        prune_tree = kwargs.get('prune_tree', False)
        mass_min = kwargs.get('mass_min')
        y = kwargs.get('discrete_outcomes') # Encoded labels expected
        ...
        # Assuming y is numeric encoded for CART, but we might want class names for rules
        # If y was passed as encoded, we can't infer names easily unless passed
    ...
    def _extract_boxes(self, triple_rules):
        ...
        # Simplify structure: take first box per label
        final_boxes = {}
